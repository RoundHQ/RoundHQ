...
import { handleRealtimeSessionStart } from "@/lib/ai-receptionist/realtime/handlers";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await handleRealtimeSessionStart({
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error && error.message.trim()
            ? error.message
            : "Unable to start AI Receptionist realtime session.",
      },
      { status: 500 }
    );
  }
